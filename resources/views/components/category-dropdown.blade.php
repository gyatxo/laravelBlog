<div x-data="{ show: false }" @click.away="show=false">
    ...
    <div x-show="show" class="absolute z-50 py-1 mt-2 rounded-xl w-full bg-gray-100 max-h-52 overflow-auto"
        style="display: none;">
        ...
        {{ request()->routeIs('home') ? 'active' : '' }}">All</a>
        @foreach ($categories as $category)
            <a href="/?category={{ $category->slug }} & {{ http_build_query(request()->except('category', 'page')) }}"
                class="rounded-xl block text-left px-3 text-sm leading-6 hover:bg-blue-500 focus:bg-blue-500 hover:text-white focus:text-white
        {{ isset($currentCategory) && $currentCategory->is($category) ? 'bg-blue-700 text-white' : '' }}">{{ ucwords($category->name) }}</a>
        @endforeach
    </div>
</div>
