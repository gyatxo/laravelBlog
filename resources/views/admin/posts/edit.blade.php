        ...
        <div class="flex">
            ...
            <main class="flex-1">
                <form action="/admin/posts/{{ $post->id }}" method="post"
                    class="max-w-xl mx-auto border border-gray-200 p-6 rounded-xl" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <div class="mb-2">
                        <label for="title" class="block mb-2 uppercase font-bold text-xs text-gray-700">Title</label>
                        <input type="text" class="border border-gray-400 p-2 w-full" name="title" id="title"
                            aria-describedby="helpId" placeholder="Title" value="{{ old('title', $post->title) }}">
                        @error('title')
                    ...
                    <div class="mb-2">
                        ...
                        <textarea name="excerpt" id="excerpt" class="border border-gray-400 p-2 w-full" rows="3">{{ old('excerpt', $post->excerpt) }}</textarea>
                        @error('excerpt')
                            <p class="text-red-500 text-xs mt-1"> {{ $message }}</p>
                        @enderror
                    </div>
                    <div class="mb-2">
                        ...
                        <textarea name="body" id="body" class="border border-gray-400 p-2 w-full" rows="5">{{ old('body', $post->body) }}</textarea>
                        @error('title')
                            <p class="text-red-500 text-xs mt-1"> {{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <button type="submit"
                        class="bg-blue-500 text-xs text-white uppercase font-semibold py-2 px-10 rounded-2xl hover:bg-blue-600">Update
                    </button>
                </form>
            </main>
        </div>
